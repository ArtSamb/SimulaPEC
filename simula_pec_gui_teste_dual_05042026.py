...
def simular_percentual_rejeicao_vetorizada(
    ...
    for idx, n in enumerate(tamanhos):
        amostras = np.random.choice(base, size=(n_iter, n), replace=True)
        
        desvios = np.std(amostras, axis=1, ddof=1)
        qui_calc = ((n - 1) * (desvios ** 2)) / (erro_padrao_precisao ** 2)
        qui_tabela = chi2.ppf(1 - (percentual_limite / 100), df=n - 1)
        rejeicoes_p = np.sum(qui_calc > qui_tabela)
        
        acima = np.sum(np.abs(amostras) > erro_admissivel, axis=1)
        porcentagem_acima = (acima / n) * 100
        rejeicoes_n = np.sum(porcentagem_acima > percentual_limite)
...
class SimulaPECApp:
    def __init__(self, master, lang='pt'):
        ...
        master.title(f"SimulaPEC {'Teste' if lang == 'pt' else 'Test'} 05/04/2026")
    ...
    def _processar_simulacao(self):
        try:
            ...
            tempo_unit = simular_percentual_rejeicao_vetorizada(
            ...
            for idx, perc in enumerate(self.percentuais):
                ...
                pr_p, pr_n = simular_percentual_rejeicao_vetorizada(
            ...
            if self.dados_reais:
                ...
                pr_p_r, pr_n_r = simular_percentual_rejeicao_vetorizada(
...
def main():
    ...
    def on_lang_selected(event):
        if selector.selected_lang:
            ...
            new_root = tk.Tk()
            new_root.title(f"SimulaPEC {'Teste' if selector.selected_lang == 'pt' else 'Test'} 05/04/2026")
