...
class SimulaPECApp:
    def __init__(self, master):
        self.master = master
        master.title("SimulaPEC Teste 13/03/2026")
    ...
    def _processar_simulacao(self):
        try:
            ...
            perc_base = float(self.entries["% PCs acima do PEC"].get())
            intervalo_acima = float(self.entries["Intervalo acima (%)"].get())
            intervalo_abaixo = float(self.entries["Intervalo abaixo (%)"].get())
    ...
    def plotar(self):
        ...
        ax[1].set_title("Curvas de Rejeição – Norma do país")
